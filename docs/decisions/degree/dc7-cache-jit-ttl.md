...
同じ質問を 10 人が連続で投げたとき、毎回フル推論するのはコストの無駄です。しかし人事異動の直後に「この人の部下一覧」をキャッシュから返せば、古い組織図に基づく誤った結果を提供してしまいます。同様に、JIT で発行した一時認証トークン（[ID-5](../../patterns/id-identity/id5-jit-scoped-credentials.md)）の有効期間が長ければ再認証の手間は減る反面、権限剥奪後もアクセスが残るリスクが生まれます。キャッシュの積極度と資格情報の TTL を、用途のリスクに応じて調整する方法を示します。
...
| 極 | 状態 | 害 |
|---|---|---|
| 過小（消極的すぎ・TTL 短すぎ） | キャッシュなし、資格情報が即時失効 | 毎回検索・認証が発生しレイテンシが増大します。JIT 資格情報の再発行コストが高い処理では詰まりが生じます |
| 過大（積極的すぎ・TTL 長すぎ） | キャッシュを広く長く保持 | 古い検索結果を使い続けます。退職・権限変更後も JIT 資格情報が有効なまま残留し、権限超過リスクが生じます |

## 判断基準

キャッシュと資格情報 TTL は、用途のリスク特性に応じて個別に設定します。

**検索キャッシュ**

- 完全一致キャッシュをプライマリ、セマンティックキャッシュをセカンダリとして使います。
- 高リスク領域（機密情報を含む検索・副作用を伴う操作）は類似度閾値を高く設定し、TTL は短くします。
- パーソナライズ応答・時系列依存データ・機密情報を含む取得結果はキャッシュを無効化し、常にフレッシュな取得を行います。

**JIT 資格情報 TTL**

- [ID-5](../../patterns/id-identity/id5-jit-scoped-credentials.md) の用途リスクで TTL を分けます。機密データアクセスや副作用を伴う操作は分単位の短い TTL、参照のみの軽量操作は時間単位の相対的に長い TTL を割り当てます。
- 権限変更・退職・セッション終了を検知したら、TTL 期限前に資格情報を強制失効させる仕組みを設けておきます。

!!! tip "キャッシュと最小権限の整合"
    キャッシュが古い権限状態を保持すると、[ID-4 Permission Mirror & Least-of](../../patterns/id-identity/id4-permission-mirror-least-of.md) で実現した最小権限の効果が損なわれます。キャッシュ無効化と権限変更イベントを連動させてください。

## 調整の仕組み

- [OB-1 Observability Lake](../../patterns/ob-observability/ob1-observability-lake.md) でキャッシュヒット率・ミス率・TTL 期限切れ発生率を計測します。
- ヒット率が低い経路では TTL 延長や類似度閾値の緩和を検討します。ヒット率が高い経路でも、コンテンツの鮮度要件を定期的に確認することを怠りません。
- JIT 資格情報の残留を検出したらアラートを発し、失効処理を自動実行できる仕組みを整えます。
...
|---|---|---|---|
| 公開FAQ・変化の少ないデータ | 積極キャッシュ・長TTL（時間単位） | KM-1, ID-5 | 鮮度低下リスク |
| パーソナライズ応答・機密データ | キャッシュ無効・短TTL（分単位） | ID-5, ID-4, KM-7 | レイテンシ増・再認証コスト |
| 権限変更・退職イベント発生時 | キャッシュ即時無効化・JIT強制失効 | ID-5, ID-4 | 一時的な性能低下 |

```yaml
decision:
  id: DC-7
  title: "キャッシュ積極度・JIT 資格情報 TTL"
  options:
    - id: aggressive
      name: "積極キャッシュ・長TTL"
      patterns: [KM-1, ID-5]
    ...
    - id: event_driven_invalidation
      name: "イベント連動キャッシュ無効化"
      patterns: [ID-5, ID-4]
      pros: [権限変更時の即時反映]
      cons: [イベント連携の実装コスト]
      pick_when: ["権限変更頻度が高い", "退職・異動イベント"]
  default_recommendation: "低リスク参照は積極キャッシュ、機密・パーソナライズは無効化、権限変更イベントで即時失効"
```
