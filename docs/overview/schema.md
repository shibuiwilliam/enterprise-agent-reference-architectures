...
各パターンを以下の8項目で統一記述する。「価値仮説」は各パターンが企業価値のどこに効くかを明示するための項目だ。「落とし穴」は事故に直結しやすいため、独立した明示項目として設けている。
...
パターンは「どの設計圧力に応えるか」で7面に分類する。この分類は責務境界とも一致し、[リファレンスアーキテクチャ](../integration/architecture/index.md)の層構造と対応している。
...
設計圧力とは、一般的なソフトウェア設計では現れにくい、エンタープライズ固有の力のことだ。
