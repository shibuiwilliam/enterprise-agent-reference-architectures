...
各パターンを以下の8項目で統一記述する。「価値仮説」により各パターンが企業価値のどこに効くかを明示し、「落とし穴」が事故に直結するため明示項目として独立させている。
...
面1〜2が「入口と統治」、面3が「権限の忠実な伝播（設計難度が高い）」、面4〜6が「実行と知識と連携」、面7が「説明責任」である。これらを積み上げる依存関係は[依存関係と依存チェーン](../integration/dependency-chain.md)で示す。
