...
パターンは「どの設計圧力に応えるか」で7面に分類する。これは責務境界とも一致し、[リファレンスアーキテクチャ](../integration/reference-architecture.md)の層構造に対応する。
...
面1〜2が「入口と統治」、面3が「最難関の権限」、面4〜6が「実行と知識と連携」、面7が「説明責任」である。これらを積み上げる依存関係は[依存関係と組み合わせレシピ](../integration/dependencies.md)で示す。

### エンタープライズ固有の設計圧力

パターンが応える設計圧力は、一般的なソフトウェア設計とは異なるエンタープライズ固有の力である。
