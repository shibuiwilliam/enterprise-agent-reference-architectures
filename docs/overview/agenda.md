...
1. **[設計原則](principles.md)** で基本思想を理解する
2. **[パターンカタログ](../patterns/index.md)** を面ごとに閲覧し、関連パターンを特定する
3. **[選定基準（程度）](../selection/degree/index.md)** と **[選定基準（トレードオフ）](../selection/tradeoff/index.md)** で調整軸と判断基準を確認する
4. **[組み合わせレシピ](../integration/recipe.md)** で実証済みのパターン組み合わせを参照する
5. **[部門別適用例](../integration/departments/index.md)** で自部門の具体例を確認する

### コーディングエージェント向け

本リファレンスは、コーディングエージェント（Claude Code、Cursor、GitHub Copilot Workspace 等）がAIエージェントを組み込んだソフトウェアの設計・開発を補助する用途を想定している。

1. **まず読む**: [`agent-digest.md`](../agent-digest.md) — 全パターンの一覧・選定基準・依存グラフ・インターフェイスを1ファイルに集約したダイジェスト
2. **構造化データ**: [`patterns-index.yaml`](../patterns-index.yaml) — 全パターンのメタデータ（requires, applies_when, interfaces）を YAML で提供。シナリオに合うパターンを `applies_when` でフィルタし、`requires` で依存を解決できる
3. **判断支援**: [`selection-rules.yaml`](../selection-rules.yaml) — トレードオフ（TO-1〜TO-12）と程度基準（DC-1〜DC-9）の判断ルールを条件式で提供。ユーザーの制約を評価して推奨を返せる
4. **詳細設計**: 個別パターンページの `## Interfaces` セクションに、実装すべきインターフェイスの YAML 定義がある。ここからスタブコードを生成できる
5. **一括取り込み**: ファイルアップロード型ワークフローでは `agent-digest.md` を使用する。さらに大きなバンドルが必要な場合は `assets/full-reference.md`（リポジトリルート）を使用する

!!! tip "コーディングエージェントへのヒント"
    ユーザーが「〇〇のシナリオでAIエージェントを設計して」と依頼したら、まず `patterns-index.yaml` を読み込み、`applies_when` でシナリオに合致するパターンをフィルタし、`requires` で依存を解決してから、該当パターンの詳細ページを読むのが効率的である。
...
`docs/scenarios.yaml` を読み込み、`sales_agent` シナリオを特定する。

→ 結果: minimum = [EX-1, ID-2, ID-4, RT-5, RT-4, OB-2, GV-1, ID-6, ...]

**Step 2 — 依存関係を解決する**

`docs/patterns-index.yaml` で各パターンの `requires` を確認する。たとえば ID-2 は EX-1 と ID-6 を必要とする。minimum リストに含まれていない依存パターンがあれば追加する。

→ 結果: 依存解決済みの最小パターンセット

**Step 3 — パターン詳細を読む**

各パターンページの `## Interfaces` セクションから YAML 定義を取得する。`code_examples` の TypeScript/Python 型定義からスタブコードを生成する。

**Step 4 — 選定基準を評価する**

`docs/selection-rules.yaml` の `input_schema` に沿って変数を設定する:
...
TO-1 を評価 → recommendation = obo。DC-1 を評価 → CRM 書き込みは tier 3 → require_approval。

**Step 5 — アーキテクチャを生成する**

パターンの設計内容を組み合わせ、システム構成図と実装計画を生成する。

## 中心命題

エンタープライズにAIエージェントを組み込む中心課題は「**AIを賢くすること**」ではない。「**企業の既存のID・権限・責任・業務プロセス・監査・データ境界・組織構造の中に、新しい実行主体を安全に参加させ、売上・生産性・意思決定の向上という企業価値を引き出すこと**」——これが本質だ。安全に参加させることは前提条件にすぎず、目的はあくまで企業価値の向上にある。

エンタープライズAIエージェントは単なるチャットUIではない。組織の権限構造を忠実に投影し、既存システム（System of Record）を壊さずに束ね、すべての行為を企業横断で監査・統治できる形に閉じ込めた**管理可能・監査可能・権限制御された「デジタル業務主体」**だ。その安全な檻の中で解き放たれた知能が、受注率向上・業務自動化・意思決定加速・コスト最適化という**企業価値を創出する実行主体**として機能する。

「誰の権限で・どのデータを・どう守って・誰の責任で」動かすかという統制設計（本書の7面・45パターン）と、「何の成果KPIを・どの経路で・いつまでに」動かすかという価値設計（[部門別適用例](../integration/departments/index.md)・[定着・アダプション](../integration/adoption.md)・[AI投資ポートフォリオ](../integration/portfolio.md)）は車の両輪だ。

### 価値ループ：創出→計測→定着→再投資

統制設計（7面・45パターン）が安全な実行基盤を提供し、その上で価値が4ステップを循環する。このループを回し続けることが、企業価値向上の実体だ。
...
一般的なAIチャットが「回答主体」であるのに対し、エンタープライズエージェントは「業務実行主体」だ。企業システム上の**一級オブジェクト**として定義・管理することになる。
...
企業はフラットなユーザー集合ではない。権限・メモリ・ログ・評価・コストはすべて、組織の階層に紐づけて設計するものだ。
...
この構造を、Workday（組織・職位・レポートライン）・Okta/Entra ID（グループ）・Linear/Asana/Jira/Notion（プロジェクト）から名寄せした単一の**組織グラフ（Org Graph）**として管理する。全面がスコープ・委譲・共有・承認の根拠をここから引くかたちになる。
...
エンタープライズでは、これらを「アプリ設計の指針」ではなく「**企業アーキテクチャ設計の制約**」として扱う。そこが重要な点だ。
