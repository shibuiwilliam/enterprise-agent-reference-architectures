...
金融の顧客情報取扱い規制、医療の PHI アクセス制限、上場企業のインサイダー情報管理など、業界特有の規制・慣習・監査要件を再利用可能なポリシーパックとしてコード化し、エージェント基盤に組み込むパターンである。規制を個々のエージェントのプロンプトに埋め込むのではなく、Policy-as-Code（ID-7）・評価パイプライン（GV-7）・データ分類・承認ルールとして実行基盤レベルで強制する。
