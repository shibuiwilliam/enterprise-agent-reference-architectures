---
title: "面2 制御・ガバナンス（GV）"
---

# 面2：制御・ガバナンス（GV）

統治と統制。一元レジストリ・モデル統制・評価・コスト・事故対応を担う。

## この面のパターン

- [GV-1 Enterprise Agent Control Plane（レジストリ／ライフサイクル）](gv1-agent-control-plane.md)
- [GV-10 Three-Layer Value Measurement（採用定着×生産性×経営KPI）](gv10-two-layer-value-measurement.md)
- [GV-2 Agent Catalog & Marketplace（社内カタログ）](gv2-agent-catalog-marketplace.md)
- [GV-3 Department Agent Factory（役割テンプレート工場）](gv3-department-agent-factory.md)
- [GV-4 Industry Policy Pack（業界ポリシーパック）](gv4-industry-policy-pack.md)
- [GV-5 Central Model Gateway（モデル・ベンダー統制）](gv5-central-model-gateway.md)
- [GV-6 Version Registry（モデル/プロンプト/ツール/ポリシー/索引の版管理）](gv6-version-registry.md)
- [GV-7 Evaluation & Governance Pipeline（評価CI/CD）](gv7-evaluation-governance-pipeline.md)
- [GV-8 Cost Quota & Chargeback（コスト配賦）](gv8-cost-quota-chargeback.md)
- [GV-9 Incident Response & Kill Switch（事故対応・停止）](gv9-incident-response-kill-switch.md)
