...
機密データの誤送信・プロンプトインジェクション・ツール暴走・コスト暴走が発生したとき、モデル・エージェント・ツール・テナントの粒度で即座に停止できる仕組みを整備するパターンである。検知→封じ込め→トレース保全→影響評価→通知→是正→ポストモーテムのフローを事前に設計し、「止められない・調べられない・影響範囲が分からない」という事態を防ぐ。全体停止しかできない設計では、1つのエージェントの問題で全社の AI が止まる。
