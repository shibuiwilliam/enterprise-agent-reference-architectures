...
審査済みのエージェント・スキル・ツールを社内アプリストアとして公開し、従業員が目的・所有者・リスク階層・データアクセス範囲・コスト・品質スコアを確認したうえで利用申請できるようにする。GV-1（Control Plane）のレジストリを土台に、「どんなエージェントが社内にあるか分からない」「別部門が同じものを作っている」「無審査で使い始める」といった問題を解消する。
