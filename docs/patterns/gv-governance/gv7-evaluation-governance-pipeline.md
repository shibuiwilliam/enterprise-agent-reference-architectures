...
LLM エージェントは非決定論的であるため、従来の単体テスト（入力に対して出力が一致するか確認する）では品質劣化を検出できない。プロンプトの一語変更、モデルのバージョンアップ、RAG 索引の更新が意図しない挙動変化をもたらすことがある。従来のテストが通っていれば変更がそのまま承認されてしまうのが問題だ。本番デプロイ後もドリフト（時間とともに挙動が劣化する現象）は起きるが、継続監視なしでは気づけない。プロンプトインジェクション・ジェイルブレイク・データ漏洩パスといった攻撃は通常テストでは発見できず、レッドチーミングが欠かせない。業務適合性と安全性を評価せずに技術指標だけで変更を承認する運用は、品質劣化が静かに積み重なる原因になる。
...
評価パイプラインは変更要求を起点として段階的に進む。各ゲートで合否が判定され、不合格の場合は前段にフィードバックが返る。本番監視フェーズは常時稼働してドリフトを継続的に検出する。
