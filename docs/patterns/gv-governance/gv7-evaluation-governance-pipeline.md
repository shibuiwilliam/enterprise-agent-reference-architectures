...
エージェントの品質・安全性・業務適合性を、単発テストではなく CI と本番の両方で継続的に測り続けるパイプラインである。変更要求からオフライン評価・セキュリティ評価・ポリシー評価・シャドーデプロイ・カナリアリリース・本番監視・フィードバックまでを一連のフローとして設計する。LLM の出力は非決定論的であるため、従来の単体テストでは品質劣化や挙動変化を検出できない。ルーブリック・LLM-as-Judge・レッドチーミング・人手レビューを組み合わせて評価する。
