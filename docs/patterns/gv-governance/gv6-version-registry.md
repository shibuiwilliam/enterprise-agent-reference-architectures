...
モデル・プロンプト・ツールスキーマ・ポリシー・RAG 索引・データスキーマをバージョン管理し、すべての変更を PR レビュー・評価・カナリアデプロイ・ロールバックの対象にする。エージェントの「デプロイ＝挙動の変更」であり、コードだけ版管理してプロンプトやモデル版を野放しにすると、小さな変更でサイレントに品質が劣化し、インシデント時に当時の挙動を再現できなくなる。
