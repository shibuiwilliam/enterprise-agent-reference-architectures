...
エージェント導入時にSaaS統合をゼロから実装し直すのは、既存の統合資産を無視した二重投資である。多くの企業はすでに MuleSoft・Workato・Boomi・社内 ESB 等の iPaaS（Integration Platform as a Service）でSaaS統合フローを構築・運用している。このパターンでは、新規に必要な統合は MCP（Model Context Protocol）で追加し、既存統合は iPaaS の呼び出しをラップして再利用するハイブリッド設計を取る。統合資産の保守を一元化し、エージェントのツール層を薄く保つ。

## 設計

エージェントのツール呼び出しは [IN-1 Tool/MCP Gateway](in1-tool-mcp-gateway.md) を経由する。Gateway は新規統合を MCP サーバーとして直接呼ぶ。既存統合については iPaaS のAPI（またはTrigger Webhook）をラップした MCP アダプターを介して呼び出す。こうして既存 iPaaS フローの更新はエージェント側に影響しない。
...
既存 iPaaS フローを MCP アダプターでラップする際は、フローの入出力インターフェースのみをエージェント向けに整形し、フロー内部のビジネスロジック・変換・エラーハンドリングは iPaaS 側に残す。

## 解決する企業課題

iPaaS で運用中の統合フローは、接続設定・変換ロジック・エラーハンドリング・監視の4つが作り込まれた資産である。これをエージェント導入のたびに作り直すと、同じSaaS接続を2箇所で保守する状態になり、変更・障害対応・セキュリティパッチの適用がすべて二重化する。ハイブリッド再利用はこの重複を排除し、既存チームの保守スキルと運用知識を引き継ぐ。

## 向き／不向き

| 向き | 不向き |
|---|---|
| 既に MuleSoft/Workato/Boomi 等が稼働しており統合フローが多数ある | エージェント導入が初めての統合でありiPaaS自体がない |
...
- **MCP Adapter**：iPaaS のAPIを MCP ツール仕様に変換する薄いラッパー
...
- [IN-1 Tool / MCP Gateway](in1-tool-mcp-gateway.md) — iPaaS アダプターを含む全ツール呼び出しの統合入口
- [IN-2 SaaS Connector / Adapter](in2-saas-connector-adapter.md) — 新規 SaaS 接続における MCP 直接実装との対比
- [ID-4 Permission Mirror & Least-of](../id-identity/id4-permission-mirror-least-of.md) — iPaaS 経由時の権限忠実性の確認
- [ID-2 Identity Federation & OBO](../id-identity/id2-identity-federation-obo.md) — iPaaS フロー経由でも本人権限を伝播するための委譲設計
