...
SaaS 統合をゼロから作り直すのではなく、企業が既に運用している MuleSoft・Workato・Boomi・社内 ESB の統合フロー・変換ロジック・認証設定をそのまま再利用するパターンである。新規に必要な統合は MCP で追加し、既存統合は iPaaS の呼び出しをラップするハイブリッド構成を取る。ただし、iPaaS の認可粒度がユーザー単位の権限忠実性（ID-4）を満たすかを事前に検証する必要がある。
