...
エージェント導入時に SaaS 統合をゼロから実装し直すのは、既存の統合資産を無視した二重投資である。多くの企業はすでに MuleSoft・Workato・Boomi・社内 ESB 等の iPaaS（Integration Platform as a Service）で SaaS 統合フローを構築・運用している。このパターンでは、新規に必要な統合は MCP（Model Context Protocol）で追加し、既存統合は iPaaS の呼び出しをラップして再利用するハイブリッド設計を取る。統合資産の保守を一元化し、エージェントのツール層を薄く保つ。

## 解決する企業課題

iPaaS で運用中の統合フローは、接続設定・変換ロジック・エラーハンドリング・監視の4つが作り込まれた資産である。これをエージェント導入のたびに作り直すと、同じ SaaS 接続を2箇所で保守する状態になり、変更・障害対応・セキュリティパッチの適用がすべて二重化する。
...
エージェントのツール呼び出しは [IN-1 Tool/MCP Gateway](in1-tool-mcp-gateway.md) を経由する。Gateway は新規統合を MCP サーバーとして直接呼ぶ。既存統合については iPaaS の API（または Trigger Webhook）をラップした MCP アダプターを介して呼び出す。こうして既存 iPaaS フローの更新はエージェント側に影響しない。
...
既存 iPaaS フローを MCP アダプターでラップする際は、フローの入出力インターフェースのみをエージェント向けに整形し、フロー内部のビジネスロジック・変換・エラーハンドリングは iPaaS 側に残す。アダプターはインターフェース変換のみを担い、ロジックは iPaaS 側に留めることで二重保守を防ぐ。
