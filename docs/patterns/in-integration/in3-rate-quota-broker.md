...
数万人が同じ SaaS を利用するエンタープライズでは、一部門のバッチ処理が Salesforce や ServiceNow の API レート上限を使い切り、全社員が 429（Too Many Requests）を受ける事態が起きる。Rate / Quota Broker は SaaS ごとにトークンバケットを持ち、対話（優先高）とバッチ（優先低）の優先度付きキュー・テナント間の公平配分・429 発生時の集中リトライを担う調停レイヤーである。各エージェントが個別にリトライする設計は SaaS を叩き潰す原因になる。
