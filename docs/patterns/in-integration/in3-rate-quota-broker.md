...
数万人が同じ Salesforce を使うエンタープライズで、ある部門のバッチ処理が API レート上限を使い切ると、全社員が 429（Too Many Requests）を受ける。このパターンは、SaaS ごとにトークンバケットを持ち、対話（優先高）とバッチ（優先低）で公平に枠を配分し、429 発生時は集中リトライで制御する調停レイヤーである。各エージェントが個別にリトライする設計は SaaS を叩き潰す原因になる。
