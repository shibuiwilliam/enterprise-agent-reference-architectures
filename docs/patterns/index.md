...
各パターンは共通スキーマ（**概要 / 設計 / 解決する企業課題 / 向き・不向き / 要素技術・既存システム連携 / 落とし穴・選定の勘所 / 関連パターン**）で記述している。スキーマの定義は[項目設計](../overview/schema.md)を参照。
...
!!! info "最小安全セット：まずここから始める"
    ...
    この最小セットで開始し、業務要件の成熟に応じてガバナンス（GV-5/GV-7）→ 実行高度化（RT-3/RT-4/RT-7）→ 知識拡充（KM-2/KM-4）の順で拡張するのが推奨ルートだ。詳細は[成熟度ロードマップ](../integration/architecture/index.md)で確認できる。
