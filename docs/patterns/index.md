...
パターンは「どの設計圧力（漏洩・サイロ・動的文脈・監査・コスト）に応えるか」で7面に分類している。これは責務境界とも一致し、[リファレンスアーキテクチャ](../integration/reference-architecture.md)の層構造に対応する。
...
!!! tip "読み方"
    面1〜2が「入口と統治」、面3が「最難関の権限」、面4〜6が「実行と知識と連携」、面7が「説明責任」。積み上げる依存関係は[統合](../integration/dependencies.md)を参照。
