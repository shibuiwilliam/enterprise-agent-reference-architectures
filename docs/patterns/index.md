...
パターンは「どの設計圧力（漏洩・サイロ・動的文脈・監査・コスト）に応えるか」で7面に分類しています。この分類は責務境界とも一致しており、[リファレンスアーキテクチャ](../integration/architecture/index.md)の層構造に対応します。

各パターンは共通スキーマ（**概要 / 設計 / 解決する企業課題 / 向き・不向き / 要素技術・既存システム連携 / 落とし穴・選定の勘所 / 関連パターン**）で記述しています。スキーマの定義は[項目設計](../overview/schema.md)を参照してください。
...
!!! tip "読み方"
    面1〜2が「入口と統治」、面3が「権限の忠実な伝播（設計難度が高い）」、面4〜6が「実行と知識と連携」、面7が「説明責任」です。積み上げる依存関係は[依存関係と依存チェーン](../integration/dependency-chain.md)を参照してください。

!!! info "最小安全セット：まずここから始める"
    45パターンを一度に導入する必要はありません。以下の6パターンが「最小安全セット」であり、これだけで参照系エージェントを安全に稼働させ、段階的に拡張できます。
    ...
    この最小セットから始め、業務要件の成熟に応じてガバナンス（GV-5/GV-7）→ 実行高度化（RT-3/RT-4/RT-7）→ 知識拡充（KM-2/KM-4）の順で拡張するのが推奨ルートです。詳細は[成熟度ロードマップ](../integration/architecture/index.md)で確認できます。
