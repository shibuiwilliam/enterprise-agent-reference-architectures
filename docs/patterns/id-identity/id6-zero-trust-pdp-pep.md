...
risk_tiers: [2, 3, 4, 5]
decision_keys: [DC-1, DC-6, TO-12]
value_drivers: [audit_compliance, automation]
kpis: ["認可判定の平均レイテンシ", "ポリシー違反検知率"]
maturity_stage: foundation
mvp: "中央PDP(OPA/Cedar)＋主要APIゲートウェイにPEPを配置"
cost_orientation: M
...
「社内ネットワークだから安全」という前提は、エージェントの世界では通用しません。プロンプトインジェクションでエージェントが乗っ取られたら、認可済みセッションを悪用して内部 API に到達できてしまいます。このパターンは、すべての行為を毎回「誰が・どのエージェントで・どのデータに・今この瞬間に許可されるか」で検証します。認可判断は中央の PDP（OPA/Cedar）に集約し、Gateway・ランタイム・コネクタの各実行点が PEP として判断結果を強制します。NIST SP 800-207 準拠のゼロトラスト認可基盤です。

## 解決する企業課題

従来のアクセス制御は「一度認証を通過したら信頼する」モデルに基づいていました。VPN 接続中なら社内リソースにアクセスできる、認証済みユーザーのセッションは継続的に信頼する——これが標準的な設計でした。エージェントがこのモデルの上で動くと、深刻な問題が発生します。

第一は「内部権限の横展開」です。一度認可を受けたエージェントが、その後のツール呼び出しや下流 API アクセスを検証なしに実行できると、プロンプトインジェクションで攻撃者がエージェントを乗っ取ったとき、認可済みセッションを悪用して本来アクセスできないデータに到達できてしまいます。

第二は「コンテキスト変化への対応不能」です。「朝に認可を受けたから夕方の操作も許可」という設計では、異動・退職・権限変更が実行中のエージェントに反映されません。ゼロトラストは毎回の検証でこのギャップを塞ぎます。

第三は「分散実行環境での制御難」です。マルチクラウド・マルチ SaaS 構成でエージェントが動く場合、各実行点がそれぞれ独自の認可判断を持つと一貫性が失われます。中央 PDP に判断を集約し、各点が PEP として強制するアーキテクチャが必要になります。

ABAC/ReBAC によるコンテキスト評価と組織グラフを属性源とする構成で、エンタープライズ AI エージェントのゼロトラスト認可を実現します。

!!! tip "最小成立条件（MVP）"
    OPA を1台立て、Gateway に PEP を1か所配置し、全エージェントリクエストを「主体×アクション×リソース」で都度認可判定します。fail-closed（不明なら拒否）を既定とします。

## 価値仮説

全アクションのリアルタイム認可により、高リスク業務領域へのエージェント適用を安全に拡大できます。適用範囲の拡大は自動化による業務コスト削減と処理速度向上に直結します。

## 解決策と設計

解決策は認可判断を中央化し、実行点を分散させることです。PDP（Policy Decision Point）が「許可か・拒否か・承認要求か」を判断し、Gateway・コネクタ・ランタイムがそれぞれ PEP（Policy Enforcement Point）として判断結果を強制します。エージェントは自律的に「これはやっていいか」を判断せず、常に PDP に問い合わせる形にします。

認可判断を中央 PDP に集約し、Gateway・コネクタ・ランタイムの各実行点が PEP として強制します。ABAC/ReBAC で主体×リソース×コンテキスト×アクションを評価し、組織グラフを属性源として活用します。判断結果は必ず監査に記録します。
...
!!! warning "PDP の単一障害点化"
    PDP を単一障害点/ボトルネックにしないでください。判断キャッシュ（短TTL）と**フェイルセーフ（不明なら拒否）**を設計します。

- PDP の判断キャッシュは短 TTL で運用します。キャッシュが長いと権限剥奪が反映されません。
- 「不明なら許可」ではなく「不明なら拒否」を既定にします（fail-closed）。
- 認可判断のレイテンシが業務に影響する場合は、PDP のレプリカ配置やエッジキャッシュで対処します。PDP を省略するという選択肢は存在しません。
- 組織グラフの鮮度は PDP の判断精度に直結します。異動・退職の反映遅延は定期的に監視しておきます。
...
decision_summary:
  ...
  recommended_if:
    ...
    - "認可判定をコードで宣言的に管理したい"
  avoid_if:
    - "単一SaaSのみでネイティブRBACで十分"
  combines_with: [ID-2, ID-4, ID-7, GV-1]
  conflicts_with: []
  value_outcome:
