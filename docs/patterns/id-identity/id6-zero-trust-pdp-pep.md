...
社内ネットワークからの呼び出しであっても無条件に信頼せず、エージェントのすべての行為を「誰が・どのエージェントで・どのデータに・どの目的で・今この瞬間に許可されるか」で毎回検証するゼロトラスト認可基盤である。認可判断を中央の PDP（Policy Decision Point、OPA/Cedar で実装）に集約し、Gateway・ランタイム・コネクタの各実行点が PEP（Policy Enforcement Point）として判断結果を強制する。NIST SP 800-207 に準拠し、組織グラフを属性源とする ABAC/ReBAC で評価する。
