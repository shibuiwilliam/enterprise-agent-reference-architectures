...
Salesforce・Box・Google Drive など各 SaaS のアクセス権限をエージェント基盤側に同期し、エージェントの実効権限を常に「エージェント能力 ∩ 本人権限 ∩ ポリシー」の交差（最小合成）に縮退させるパターンである。これにより、RAG が本来ユーザーに見えない文書を返す「サイロ越え漏洩」や、退職者・異動者の剥奪済みアクセスが残る「遅延失効」を防ぐ。OBO 委譲（ID-2）が使える SaaS では不要だが、委譲非対応の旧式 SaaS・独自システムではこのパターンが権限忠実性を担保する唯一の手段となる。
