...
従業員向けエージェントと顧客向けエージェントを、IdP・データストア・実行環境・監査ログのすべてにおいて物理的・論理的に完全分離するパターンである。社内 AI が保有する人事情報・未公開案件・内部メトリクスが顧客チャネルへ流出する事故、あるいは顧客データが社内エージェントの推論に混入する逆方向の漏洩——これらエンタープライズ最重大の漏洩クラスを、「そもそも到達できない」構造で排除する。面をまたぐデータ移動は原則ゼロとし、必要な場合のみ明示ゲート（分類・承認・マスキング）を経由させる。
