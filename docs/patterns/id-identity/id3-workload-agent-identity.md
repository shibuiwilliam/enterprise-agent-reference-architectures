...
定時バッチ・Webhook トリガー・自律ループなど、人間の依頼を直接代理しないエージェントに対し、人間 ID とは独立した検証可能なマシン ID（Workload Identity）を付与するパターンである。SPIFFE/SVID やクラウドのワークロード ID で短命の証明書を発行し、自律動作の権限を人間の代理とは別系統で管理・監査する。すべての呼び出しは「人間 ID（存在する場合）＋ワークロード ID」の二重表現で記録される。
