...
エージェントが実行する処理の中には、数分から数時間にわたる長時間処理が含まれる。人間の承認待ち、外部システムの非同期応答待ち、大量ドキュメントの一括処理などがその例である。これらをHTTPリクエスト上で同期的に実行すると、タイムアウトや障害により処理が失途中で失われる。Durable Enterprise Agent Workflowパターンは、エージェント処理の状態をステップ境界ごとに永続化し、ワーカー障害・再起動・スケールアウトを跨いで処理を継続する。LLMの出力結果はアクティビティ境界で固定されるため、リプレイ時に異なる結果が生成されるリスクを排除できる。
...
ワーカーの障害は常に起こりうる。Kubernetes Podの退避、デプロイ、インフラ障害など、実行途中でプロセスが停止するシナリオは珍しくない。永続ワークフローは状態をストアに分離するため、ワーカーが入れ替わっても処理を継続できる。

また、長時間プロセスのデバッグ・監査にも有効である。各ステップの入力・出力・実行時刻がワークフロー履歴として残るため、障害調査や規制対応の際に処理経緯を追跡できる。
...
- [RT-7 Enterprise Saga Agent](rt7-enterprise-saga.md)：SagaステップをDurable Workflow内のアクティビティとして実装し、補償フローを組み込む
- [RT-4 Human Approval Chain](rt4-human-approval-chain.md)：`waiting_approval` 状態でHitL承認を受け取る仕組みと組み合わせる
- [RT-9 Enterprise Work Queue Agent](rt9-work-queue-agent.md)：キューからタスクを取得しDurable Workflowとして処理するアーキテクチャに組み合わせる
- [OB-1 Observability Lake](../ob-observability/ob1-observability-lake.md)：ワークフロー実行状態・実行時間・コストを監視する
