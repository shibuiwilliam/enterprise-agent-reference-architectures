...
数分〜数時間かかるエージェント処理（大量文書の一括分析、人間の承認待ち、外部システムの非同期応答待ちなど）を、同期 HTTP ではなく永続ワークフローとして実行するパターンである。処理状態をステップ境界ごとに永続化し、ワーカーの障害・再起動・スケールアウトを跨いで処理を継続する。LLM の出力はアクティビティ境界で固定されるため、リプレイ時に異なる結果が生成されるリスクを排除する（リプレイ安全性）。Temporal・Step Functions・Durable Functions で実装する。
