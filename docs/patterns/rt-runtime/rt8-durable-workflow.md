...
エージェントが実行する処理の中には、数分から数時間にわたる長時間処理が含まれる。人間の承認待ち、外部システムの非同期応答待ち、大量ドキュメントの一括処理などがその例である。これらをHTTPリクエスト上で同期的に実行すると、タイムアウトや障害により処理が途中で失われる。Durable Enterprise Agent Workflowパターンは、エージェント処理の状態をステップ境界ごとに永続化し、ワーカー障害・再起動・スケールアウトを跨いで処理を継続する。LLMの出力結果はアクティビティ境界で固定されるため、リプレイ時に異なる結果が生成されるリスクを排除できる。
