...
エージェントを「チャットボット」ではなく、ServiceNow・Zendesk・Jira などの業務キューからチケットを取得して処理する「ワーカー」として設計するパターンである。人間のオペレーターと同じキューに参加し、自律的に処理を試みる。対応不可と判断した場合はキューに残すか人間にエスカレーションする。SLA 管理・負荷分散・優先度付けは既存のキュー基盤がそのまま担う。
