...
エージェントをチャットボットとして設計するのではなく、組織が既に運用している業務キュー（サポートチケット、問い合わせ、申請書、レビュー依頼など）の「ワーカー」として参加させるパターンである。人間のオペレーターと同じキューからタスクを取得し、自律的に処理を試みる。対応不可と判断した場合はキューに残すか、スキル条件を満たす人間にエスカレーションする。SLA管理・負荷分散・優先度付けは既存のキュー基盤が担う。
...
タスク取得時にエージェントは自身の処理スコープ（対応可能なカテゴリ・リスクレベル・権限範囲）を評価する。スコープ外・高リスク・判断困難なケースは即座に人間にエスカレーションする。SLA残時間が一定値を下回った場合も自動エスカレーションする。エージェントが部分処理を行った場合は、調査結果・試行内容をチケットにコメントとして記録してから引き継ぐ。これにより、担当者が引き継ぎ時に経緯を把握できる。

## 解決する企業課題

AI処理と人間業務ワークフローの断絶がこのパターンが解決する中心的な課題である。「AI専用チャット画面」を別途設けると、既存業務フロー（ServiceNow/Zendesk/Jiraで管理されているSLA・優先度・担当割り当て）から切り離された孤立した処理が生まれる。結果として、AIが処理したのかどうか、SLAが守られたかどうかが追跡できなくなる。

エージェントをキューのワーカーとして組み込むことで、AIと人間のハンドオフがキューの標準ルーティングロジックに乗る。SLA管理・エスカレーションルール・負荷分散は既存のITSMプロセスがそのまま機能する。監査の観点では、誰（AIか人間か）がいつ何を処理したかがチケット履歴として一元管理される。
...
- [RT-8 Durable Enterprise Agent Workflow](rt8-durable-workflow.md)：キューから取得したタスクをDurable Workflowとして実行し、長時間処理・承認待ちに対応する
- [RT-4 Human Approval Chain](rt4-human-approval-chain.md)：エスカレーション時の人間承認フローと組み合わせる
- [RT-10 Event-Driven Enterprise Orchestrator](rt10-event-driven-orchestrator.md)：イベントをトリガーにキューへタスクを積む構成と組み合わせる
- [EX-2 Embedded vs Portal](../ex-experience/ex2-embedded-vs-portal.md)：エージェントを既存ツールに組み込む際のUX設計の参考にする
- [OB-1 Observability Lake](../ob-observability/ob1-observability-lake.md)：エージェントのキュー処理状況・SLA達成率・エスカレーション率を監視する
