...
エージェントを個人アシスタントではなく、プロジェクトやチームに紐づく共有メンバーとして設計するパターンである。プロジェクト開始時に専用ワークスペース（GraphRAG による共有メモリ・Slack チャンネル・Jira ボード・Box フォルダ）を自動プロビジョニングし、メンバーは `@Project-X-Agent` で対話する。エージェントは受動的に呼ばれるだけでなく、毎朝 Jira の進捗と Slack の議論を突合して仕様の不整合を自律警告するなど、能動的なチームメンバーとして振る舞う。プロジェクト終了時にはメモリと権限を自動でアーカイブ・失効させる。
