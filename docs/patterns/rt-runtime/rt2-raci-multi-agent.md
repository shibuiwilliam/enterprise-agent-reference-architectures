...
マルチエージェント構成を採用する理由を「タスクが複雑だから」ではなく「企業内の責任分担（RACI）が複数に分かれるから」に置くパターンである。たとえば契約書レビューでは、Responsible（実行）= Legal Agent、Consulted（助言）= Sales/Finance/Security Agent、Accountable（最終責任）= 法務マネージャー、Informed（通知）= AE/CS というように、企業の責任構造をエージェントのトポロジに直接反映する。責任境界が曖昧なマルチエージェントでは最終責任者が不明になり、議論が発散して収拾がつかなくなる。
