...
マルチエージェントにする理由が「タスクが複雑だから」ではうまくいかない。うまくいくのは「企業内の責任分担が複数に分かれるから」である。たとえば契約書レビューなら、Legal Agent が実行し、Sales/Finance Agent が助言し、法務マネージャーが最終責任を負い、営業担当に結果を通知する——この RACI 構造をそのままエージェントのトポロジに反映する。責任境界が明確でなければ、複数のエージェントが議論だけして誰も決めない事態に陥る。
