...
RACI マトリクスをシステム設計の入力として扱う。各ロールに対応するエージェントまたは人間アクターを定義し、オーケストレーターがマトリクスに従って処理を進める。
...
オーケストレーターは各フェーズで誰が R・A・C・I であるかをデシジョンログに記録する。承認（A）が得られるまで次フェーズへ進まないゲートを設ける。C からのフィードバックは R に集約され、R が最終判断に統合する責任を持つ。

## 解決する企業課題

エンタープライズで頻出する課題は「誰が最終的な説明責任を持つか不明確」な状態である。マルチエージェントシステムでは、複数のエージェントが関与することで責任の所在が拡散しやすい。RACI に基づく設計は、Accountable を必ず人間に割り当てることで、エージェントの自律実行に対する最終責任者を明確にする。

ドメイン間の責任境界があいまいな業務（法務・財務・セキュリティが関わる契約承認など）では、各ドメインが「どこまでが自分の判断範囲か」を把握していない。RACI マトリクスをハンドオフプロトコルとして実装することで、エージェントが越権した判断を下す構造的リスクを排除する。

デシジョンログにより、誰がいつどの根拠で判断したかが監査可能になる。規制対応（SOX、個人情報保護法など）での説明責任要求に応えられる。
...
- [RT-1 Org-Hierarchical Hub & Spoke](rt1-org-hierarchical-hub-spoke.md)
- [RT-4 Human Approval Chain](rt4-human-approval-chain.md)
- [GV-1 Enterprise Agent Control Plane](../gv-governance/gv1-agent-control-plane.md)
