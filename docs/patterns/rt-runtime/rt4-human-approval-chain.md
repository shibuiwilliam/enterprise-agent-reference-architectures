...
高リスク・不可逆な操作（契約変更・金銭処理・人事異動・権限変更など）の実行前に、組織グラフから承認者を動的に解決して人間の承認を挟むパターンである。エージェントは「提案」まで行い、「実行」は承認後に限定する。承認者のハードコードは異動で宛先不明になるため、Workday の組織図から上長・所管責任者・コスト所有者を都度解決する。不在時の代理（delegation of authority）、SLA タイマーによる期限管理、却下理由のフィードバックまで設計する。
