...
エージェントは「この契約を更新しますか？」と提案するまでが仕事。実行は人間の承認後である。このパターンでは、組織グラフ（Workday の組織図）から上長・所管責任者・コスト所有者を動的に解決して承認を求める。承認者をコードに直書きすると異動で宛先不明になるため、常に組織グラフから都度解決する。Slack や ServiceNow での承認体験、不在時の代理委譲、SLA タイマー、却下理由のフィードバックまで含めて設計する。
