...
エージェントの操作を可逆性・影響範囲・データ分類に基づいてリスク Tier に分類し、Tier ごとに自律実行・単独承認・二者承認・禁止を強制するパターンである。「すべて自動」はリスクが高すぎ、「すべて承認」は業務が停滞する。この二項対立を解消し、低リスク操作は自動化、高リスク操作は人間の介在を保証する。
...
リスクスコアリングはポリシーエンジン（ID-7）が担う。操作の対象リソースのデータ分類、操作の不可逆性（削除・送信・支払いなど）、影響を受けるユーザ・組織の範囲を入力として Tier を決定する。Tier は固定値ではなく、文脈によって動的に変わりうる。

## 解決する企業課題

「全操作を承認必須にする」運用は承認待ちのボトルネックを生み、エージェント活用の価値を損なう。一方「全操作を自動化する」設計は、資金移動・権限付与・顧客送信の誤実行リスクを抱える。Tier 設計はこのトレードオフを明示的に解決する。

部門ごとにリスク感覚が異なる企業では、「このエージェントがここまで自動でやっていいのか」という基準が属人化しやすい。Tier 定義をポリシーとして明文化・強制することで、部門横断的な一貫性を確保する。

読み取り操作（Tier 0）はほぼすべての業務で自動化できる。これだけでも大きな業務効率化が得られ、高リスク操作への承認リソースを集中できる。
...
- [ID-6 Zero-Trust PDP/PEP](../id-identity/id6-zero-trust-pdp-pep.md)
- [GV-7 Evaluation & Governance Pipeline](../gv-governance/gv7-evaluation-governance-pipeline.md)
