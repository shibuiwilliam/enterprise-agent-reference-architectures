...
エージェントが実行する操作をリスクに応じて Tier 0（回答・要約のみ）から Tier 5（禁止または二者承認必須）まで段階化し、Tier ごとに自律実行・単独承認・複数承認・禁止を強制するパターンである。社内記録の参照は自動で処理し、顧客への通知は単独承認、金銭・契約・人事の変更は二者承認を要する——このように「全自動は危険、全承認は遅い」という二項対立を解消し、リスクに見合った統制をポリシーで適用する。
...
部門ごとにリスク感覚が異なる企業では、「このエージェントがここまで自動でやっていいのか」という基準が属人化しやすい。コスト・監査・コンプライアンスの観点で問題が表面化するのは、承認なしで実行された操作が後から問題になる場面が多い。特に金銭・人事・顧客データに関わる操作は、一度実行すると取り消しが困難な不可逆性を持つ。
...
リスクスコアリングはポリシーエンジン（ID-7）が担う。操作の対象リソースのデータ分類、操作の不可逆性（削除・送信・支払いなど）、影響を受けるユーザ・組織の範囲を入力として Tier を決定する。Tier は固定値ではなく、文脈によって動的に変わりうる。同じ「社内記録への書き込み」でも、対象が個人情報を含む場合は Tier 4 相当に引き上げられる。
