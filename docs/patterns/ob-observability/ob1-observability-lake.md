...
エージェントの実行ログ・分散トレース・トークン消費・ツール呼び出し・RAG 取得文脈・承認状態・品質評価結果を統合し、「なぜその判断をしたか」を後から再構成可能にする観測基盤である。保存は三層に分離する——メタデータ（モデル版・コスト・レイテンシ・相関 ID）は Trace DB、本文（プロンプト・取得文脈・成果物）は PII マスキング済みの暗号化ストレージ、集計指標は DWH。OpenTelemetry GenAI semantic conventions に準拠する。
