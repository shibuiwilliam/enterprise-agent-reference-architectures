...
エージェントに渡す文脈を「アクセスできるデータ全部」ではなく「この業務目的に必要な最小データ」に限定するパターンである。営業フォローアップ・契約レビュー・サポート対応・オンボーディングなど目的ごとに、許可データ種別・接続システム・保持期間・マスキング要件をポリシーとして定義し、トークン予算内に収まるバージョン管理されたコンテキストパッケージを生成する。無関係なデータの詰め込みは精度低下（lost in the middle）とコスト増を招く。
