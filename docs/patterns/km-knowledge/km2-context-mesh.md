...
Salesforce の案件情報も Workday の人事データも「一箇所に集めれば便利」に見えるが、コピーした時点で元の権限モデルは崩れる。このパターンは、データを集約せず、本人の OBO トークンで各 SaaS に分散問い合わせ（フェデレーション）してリアルタイムに文脈を取得する。公開情報は中央ベクトル DB でインデックス化し、機密 SaaS データは JIT 取得するハイブリッド構成が実務的な解である。データ所在地規制への対応もしやすい。
