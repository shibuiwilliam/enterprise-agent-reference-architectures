...
Salesforce・Slack・Google Drive・Jira など各 SaaS のデータを一箇所のベクトル DB に集約せず、本人の OBO トークンで各データ源に分散問い合わせ（フェデレーション）してリアルタイムに文脈を取得するパターンである。データをコピーしないため元の権限モデルが維持され、データ所在地規制にも対応しやすい。公開社内規程は中央 DB でインデックス化し、機密 SaaS データは JIT 取得するハイブリッド構成が実務的な解となる。
