...
エージェントの機密漏洩経路は「LLM への入力」だけではない。LLM の出力・RAG 結果・ツール実行結果・ログ保存——計5つの境界すべてにマスキングを配置しなければ穴が残る。このパターンは、PII・秘密鍵・契約金額などを DLP で検出し、マスキングまたはトークナイゼーションで除去する。最高機密データは外部 LLM への送信自体を禁止し、社内推論基盤へルーティングする。
