...
人事評価・M&A 検討・インサイダー情報——これらは「ログにも残さない」レベルの機密性が求められる。このパターンは、コンテキストを機密計算の隔離領域（AWS Nitro Enclaves 等）で処理し、セッション終了と同時に物理破棄する。DLP（KM-6）が「機密を見つけて消す」アプローチなら、こちらは「最初から何も残さない」揮発アプローチである。ログ基盤にはレイテンシやトークン数などのメタデータだけを送る。
