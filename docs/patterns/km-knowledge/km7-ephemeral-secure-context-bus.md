...
人事評価・M&A 検討・インサイダー情報など、ログやキャッシュに一切平文を残せない極秘処理のために、コンテキスト（プロンプト・記憶・中間結果）を機密計算の隔離領域（AWS Nitro Enclaves / Azure Confidential Computing）で処理し、セッション終了と同時に物理破棄するパターンである。DLP（KM-6）が「汚染除去」アプローチなのに対し、本パターンは「最初から残さない」揮発アプローチを取る。ログ基盤にはレイテンシ・トークン数・コストなどのメタデータのみを送信する。
