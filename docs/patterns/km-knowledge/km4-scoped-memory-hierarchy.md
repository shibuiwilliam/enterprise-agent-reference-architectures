...
エージェントにメモリを持たせると便利ですが、「個人のメモリがチーム全体に見える」「A プロジェクトの顧客情報が B プロジェクトに漏れる」という事故が起きます。このパターンはメモリを個人・チーム・プロジェクト・部門・全社・顧客の各スコープに分離し、共有範囲を組織グラフに従わせます。退職やプロジェクト終了に連動してメモリと権限を自動で失効させ、本人が自分のメモリを消去できる権利も設計に含めます。

## 解決する企業課題

エージェントにメモリを持たせると過去の文脈を再利用できます。その反面、「誰がどの記憶を参照できるか」を管理しなければ情報漏洩の経路になります。個人メモリがチーム全体に見える、A プロジェクトで得た顧客情報が B プロジェクトのエージェントに参照される、退職した担当者が持っていた文脈が後任に見える——これらはスコープなし設計で発生する典型的な問題です。

企業の組織構造は、それ自体が情報共有の権威ある基準となります。「同じチームのメンバーは同じ情報を見てよい」「部門長は部門内のプロジェクト情報を見てよい」——こうした組織の論理をメモリ階層に反映させれば、権限管理を組織グラフという既存の権威ソースに委ねられます。プロジェクト終了・退職・異動といったライフサイクルイベントに連動してメモリを失効させることで、陳腐化した文脈の誤用も防げます。

!!! tip "最小成立条件（MVP）"
    Vector DB の Namespace を Personal / Team / Company の3層に分け、書き込み時にスコープを付与します。組織グラフ連動や自動失効は後続フェーズでよいですが、スコープ分離だけは初期から入れます。

## 価値仮説

チーム・プロジェクト単位の記憶共有により、ナレッジの属人化を解消します。暗黙知の共有は新人の立ち上がり時間短縮とチーム生産性向上に寄与します。

## 解決策と設計

各スコープを物理的・論理的に分離し、書き込みはゲート（分類・重複検知・承認）を経由させます。サブプロジェクトは親の非機密情報のみ継承し、承認者は種別ごとに分けます（PM / 部門責任者 / 顧客情報管理者）。
...
スコープの境界は Vector DB の Namespace や暗号化キーで物理的に分離します。プロジェクト終了・退職・異動でメモリと権限を失効させる処理は自動化しておきましょう。本人が自分のメモリを確認・消去できる Memory Review UI を提供し、Right to Erasure（消去権）を設計に組み込みます。
...
!!! warning "全社共有メモリの罠"
    すべてを「全社共有メモリ」にし機密と雑多を混在させるのは最大のアンチパターンです。スコープを分離し、共有範囲を組織グラフに従わせます。「速く作れるから全部共有」は技術負債ではなくセキュリティ上の欠陥です。

- 本人が自分のメモリを確認・消去できる権利（Right to Erasure）を設計に含めてください。規制要件（GDPR 等）への対応だけでなく、誤った情報が蓄積した場合の修正手段としても欠かせません。
- プロジェクト終了時のメモリアーカイブ/失効は自動化しておきましょう。放置すると、異動者経由で元のプロジェクト情報が漏洩するリスクが残ります。
- メモリの保持・忘却は「重要度 × 鮮度 × 参照頻度」で選別し、古い詳細は要約へ圧縮します。無限に蓄積するとノイズが増え、有用な文脈の検索精度が下がります。
...
- [KM-5 Purpose-Bound Context](km5-purpose-bound-context.md) — 補完：メモリから取り出す文脈を業務目的でさらに限定します
...
decision_summary:
  ...
  participates_in:
    ...
    - decision: DC-5
      role: primary
  recommended_if:
    - "個人・チーム・プロジェクト間でメモリスコープを分けたい"
    - "長期記憶と短期記憶を階層化する必要がある"
  avoid_if:
    - "ステートレスなQ&Aのみ"
  combines_with: [KM-1, RT-11, ID-4]
  conflicts_with: []
  value_outcome:
