...
エージェントのメモリを個人・チーム・プロジェクト・部門・全社・顧客の各スコープに物理的・論理的に分離し、共有範囲を組織グラフに従わせるパターンである。個人メモリがチーム全体に見える、A プロジェクトの顧客情報が B プロジェクトに漏れるといった問題を構造的に防ぐ。プロジェクト終了・退職・異動に連動してメモリと権限を自動失効させ、本人が自分のメモリを確認・消去できる権利も設計に含める。
