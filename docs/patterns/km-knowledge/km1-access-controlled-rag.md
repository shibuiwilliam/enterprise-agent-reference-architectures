...
社内文書を検索・RAG で活用する際に、データ源のアクセス制御（ACL）を保ったまま、依頼者が閲覧権限を持つ文書だけを検索結果に含めるパターンである。ベクトル DB にコピーした瞬間に元の権限が失われる——これが企業 RAG 最大の落とし穴であり、本パターンは取り込み時に ACL・分類・鮮度をチャンクに同梱し、検索時に最新のエンタイトルメントで再評価することで、退職者・異動者のアクセス残存を防ぐ。
