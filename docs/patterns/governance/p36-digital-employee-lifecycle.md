...
!!! abstract "メタ"
    **カテゴリ**: ガバナンス ／ **対象**: 両方 ／ **成熟度**: 標準 ／ **関連**: [P35](../organization/p35-agent-registry.md), [P08](../identity/p08-token-exchange-obo.md), [P26](../reliability/p26-eval-gate-canary.md)

**トリガ（採用検討の条件）**: エージェントが増殖する／野良エージェント・権限漸増・暴走時の停止手段の欠如が懸念される。
...
- **向き**: エージェントが増加するすべての組織（早期導入が望ましい）。
- **不向き**: エージェントが数個の初期段階（軽量版で十分）。
