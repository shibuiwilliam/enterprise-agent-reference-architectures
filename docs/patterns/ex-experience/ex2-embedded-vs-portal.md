...
エージェントの提供形態を「業務アプリへの埋め込み」と「独立ワークベンチ」の2つに分け、業務特性に応じて使い分けるパターンである。Slack でのクイック質問や Salesforce 画面上の商談サマリなど日常的な短いタスクは埋め込みで提供し、複数システムにまたがる調査・承認フロー・長時間タスクは計画・進捗・根拠・承認を一画面で確認できる独立ワークベンチで提供する。「ポータルだけ作ったが誰も開かない」問題を避け、仕事のある場所にエージェントを届ける。
