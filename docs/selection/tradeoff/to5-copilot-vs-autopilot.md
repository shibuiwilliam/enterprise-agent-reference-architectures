...
CopilotとAutopilotは自律度の異なる二つのエージェント動作モードである。経営はAutopilotのROI（Return on Investment）を求めがちだが、確率的な動作による基幹業務システム（Workday・SAP・Salesforce等）の破壊は許容できない。どちらのモードを採用するかは業務の可逆性とリスク許容度で決まる。
