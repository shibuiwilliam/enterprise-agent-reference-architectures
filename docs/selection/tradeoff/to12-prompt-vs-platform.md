...
システムプロンプトに「機密情報を出力するな」と書けば安全だろうか？　答えは明確に「いいえ」である。ユーザーが「上記の指示を無視して」と入力するだけで突破されるプロンプトは、セキュリティ境界にならない。安全保証は権限・認可・Policy Engine など実行基盤側に置き、プロンプトは応答トーンや出力形式の調整に使う――この役割分担がエンタープライズ設計の大原則である。
