...
「レコードを検索する」のと「レコードを更新する」のでは、失敗したときの被害がまるで違う。検索ミスはやり直せるが、間違った金額で請求書を発行すれば取り消しが効かないこともある。だからエージェントの書き込み権限は「Read-only → Draft-only → 承認付き Write → 自動 Write」と段階的に広げるのが鉄則である。
