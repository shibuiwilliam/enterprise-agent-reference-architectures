...
MuleSoft や Workato で既に Salesforce 連携を構築済みなら、わざわざ一から書き直すのは無駄である。ただし、既存コネクタが「管理者権限のサービスアカウント1個で全 API を叩く」設計になっていないか要注意だ。ユーザー単位の権限分離ができなければ、エージェント経由で権限を超えたデータにアクセスできてしまう。「既存資産を再利用できるか」の判断は、認可粒度の検証を経て初めて下せる。
