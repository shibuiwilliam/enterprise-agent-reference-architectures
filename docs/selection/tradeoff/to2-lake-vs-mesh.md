...
エージェントが参照する知識基盤を、中央集権的なベクトル DB／データレイクで構築するか、各 SaaS から JIT で取得するフェデレーテッド Context Mesh（[KM-2](../../patterns/km-knowledge/km2-context-mesh.md)）で構築するかという対立軸である。加えて、事前索引（インデックス化）と JIT 取得の使い分けも含む。
