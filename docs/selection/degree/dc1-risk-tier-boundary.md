...
| 極 | 状態 | 害 |
|---|---|---|
| 過小（厳しすぎ） | すべての操作に承認を要求 | 承認者がボトルネックとなり、エージェント導入の価値が消える。承認疲れで形骸化も起こる |
| 過大（緩すぎ） | 大半の操作を自動実行 | 不可逆な誤実行（金銭移動・契約変更・権限昇格・外部公開）のリスクが現実化する |

## 判断基準

ティア境界は次の4軸を組み合わせて決める。

- **影響の不可逆性**：取り消し可能な操作（ドラフト作成・参照）は自動寄り、取り消し不能な操作（送金・契約締結・外部送信）は承認寄りに設定する
- **影響額／影響範囲**：個人に閉じるか、チーム・部門・全社・社外に及ぶかで段階を分ける
- **データ機密度**：公開情報 → 社内一般 → 部門機密 → 極秘の順に厳格度を上げる
- **本人の職責**：依頼者の役職・権限レベルに応じて自律範囲を変える。同じ操作でも管理職と一般職では閾値が異なる

参照系（読み取り）は原則自動、更新系（金銭・契約・人事・権限変更・外部公開）は承認寄りに設定するのが基本方針だ。
...
- [OB-1 Observability Lake](../../patterns/ob-observability/ob1-observability-lake.md) で承認率・自動実行の成功率・インシデント発生率を継続計測する
...
- 承認率が高止まりしている Tier は自動化の候補、インシデントが発生した Tier は承認範囲を広げる
- Tier 境界の変更も変更管理（[GV-1](../../patterns/gv-governance/gv1-agent-control-plane.md)）の対象とし、監査証跡を残す
