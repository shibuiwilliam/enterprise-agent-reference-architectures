...
ガードレールの閾値をどこに設定するかを決める連続量パラメータである。閾値を上げれば見逃し（FN）が減る一方で誤検知（FP）が増え、正当な処理を過剰に遮断する。閾値を下げれば誤検知は減るが、危険な操作を通過させるリスクが高まる。[ID-7 Policy-as-Code Guardrail](../../patterns/id-identity/id7-policy-as-code-guardrail.md) の実装において、このバランスを経路ごとに調整する。
