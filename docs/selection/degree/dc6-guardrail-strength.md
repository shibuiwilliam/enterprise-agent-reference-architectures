...
ガードレールを厳しくしすぎると、正当なメール送信まで「機密漏洩の疑い」で毎回ブロックされ、ユーザーはエージェントを使わなくなる。緩めすぎれば、本当に危険な出力を素通りさせてしまう。このバランスを一律に決めることはできない。「社外向けメール送信」と「社内メモの要約」では当然リスクが異なるからだ。[ID-7 Policy-as-Code Guardrail](../../patterns/id-identity/id7-policy-as-code-guardrail.md) の閾値を経路のリスク特性ごとに調整する方法を扱う。
...
- **低リスク経路**（参照のみ・社内下書き・既承認テンプレート）は軽量なガードレールで十分。FP（誤検知）による業務阻害を最小化する
- 閾値の設定根拠は「ビジネス許容点」に置く。FP 率と FN 率をそれぞれ計測し、業務上どちらの害が大きいかで判断する
- レイテンシがクリティカルな経路では、同期ブロッキングではなく非同期・サンプリング方式を選ぶことで影響を緩和できる
...
- [GV-7 Evaluation & Governance Pipeline](../../patterns/gv-governance/gv7-evaluation-governance-pipeline.md) で FP 率・FN 率・インシデント件数を定期計測し、閾値調整の根拠とする
- [OB-1 Observability Lake](../../patterns/ob-observability/ob1-observability-lake.md) でガードレール発動件数・種別・経路を記録し、誤検知が多い経路を特定する
- [KM-6 DLP & Redaction Boundary](../../patterns/km-knowledge/km6-dlp-redaction-boundary.md) の出力フィルタリングと連動させ、コンテンツ検査の粒度を合わせる
- 閾値変更自体も変更管理の対象とし、変更前後の FP/FN 推移を比較する
