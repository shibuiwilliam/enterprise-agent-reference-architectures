...
エージェントが蓄積する記憶（会話履歴・学習結果・個人設定）を、どの程度の期間・どの範囲で保持し、いつ忘却するかを決める連続量パラメータである。[KM-4 Scoped Memory Hierarchy](../../patterns/km-knowledge/km4-scoped-memory-hierarchy.md) のスコープごとに TTL と失効条件を設定する。
