...
タスクの難易度とデータの機密分類に応じて、使用するモデルとその実行経路をどう振り分けるかを決める連続量パラメータである。強いモデルを一律に使えば品質は上がるがコストが増大し、機密データを外部 API に送ればコンプライアンスリスクが生じる。[GV-5 Central Model Gateway](../../patterns/gv-governance/gv5-central-model-gateway.md) で難易度推定とデータ分類を組み合わせたルーティングを実装する。
...
機密分類を無視したルーティングは独立した問題を引き起こす。極秘データを外部 API に送信するのは規制違反・情報漏洩のリスクになる。
