...
検索結果キャッシュの積極度と、[ID-5 JIT Scoped Credentials](../../patterns/id-identity/id5-jit-scoped-credentials.md) で発行する一時資格情報の TTL をどう設定するかを決める連続量パラメータである。積極的なキャッシュは応答速度とコストの改善をもたらすが、古い情報や権限の残留というリスクを生む。
