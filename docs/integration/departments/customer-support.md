...
カスタマーサポート部門のエージェントは、他の部門と決定的に異なる特徴を持つ。顧客（外部ユーザー）と直接やり取りしながら、社内の契約 DB・製品 DB・返金システムにも同時にアクセスする。この「顧客面と社内面が交差する」構造が独自のリスクを生む。顧客向けの ID・権限・ログと社内従業員向けのそれを混在させると、権限境界が曖昧になり不正アクセスの温床になる。FAQ 回答は完全自動でよいが、返金処理や契約変更は人間承認を要する——この「自動と承認の段階的使い分け」もカスタマーサポート特有の設計要件だ。
